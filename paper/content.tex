...
Amazon Simple Storage Service (S3) is an object storage built to store and
retrieve any amount of data from anywhere such as web sites or mobile
...
object where object can be a data file in any format. Each bucket has its own
name and this name is globally unique. Amazon has defined different regions
based on availability of its resources. The globally unique name is unique name
across all regions provided by Amazon. This global namespace allows users to
have the unique bucket globally which is useful for organizing and accessing
data.

Amazon has not specified any restriction on number of data objects to be stored
in one bucket bur number of buckets are limited to 100 per account. Each object
is associated with meta data which is in key value pair format and consists of
predefined meta data such as LastModified date time of object.

Users can access or create objects based on the access control restrictions
applied to that bucket. Amazon S3 has a search functionality which can be used
to search the objects within the bucket. This search is based on the file name
and the search string should contain the full file name or any prefix of file
name. Amazon does not provide content based search functionality for S3 so one
cannot search for the internal data in a file.
...
\item [User identities.] Each user registered to Amazon’s web service gets an
