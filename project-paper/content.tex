...
\keywords{hid-sp18-420, File system, abstraction, Amazon S3, Google
...
The general layerd acrhitecture of the Service is depicted in
...
En example shown in Figure~\ref{fig:client} identifies the ability to
...
REST file system abstraction makes them appear to be in teh same file
system.
...
List of requirments bal bl bla

\begin{description}
\item[Storing files:] bla

\item[Distributing files across different services:] bla

\item[Accessing boto, google amazon ,....:] bla

\item[Security and permissins for file access:] bla

\item[Directories:] bla

\item[Virtual directories:] bla

\end{description}

\section{Architecture}

Ondependent form the technology

OpenAPI and advantage
...
\begin{figure*}[!ht]
\centering\includegraphics[width=\columnwidth]{image/not-configured.JPG}
\begin{footnotesize}
\begin{verbatim}
root@ip$ make test
curl http://....
...
\TODO{THIS README SHOULD NOT COMMENT ON THE REPORT FORMAT PUT ON HOW
  TO RUN TEH PROGRAM}
